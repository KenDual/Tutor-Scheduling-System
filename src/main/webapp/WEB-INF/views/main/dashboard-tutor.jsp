            ...
            <main class="flex-1 p-8">
                ...
                <div id="schedule" class="section" style="display:none;">
                    ...
                    <form:form action="${pageContext.request.contextPath}/sessions/request"
                            ...
                            <form:option value="mon" label="Thứ 2"/>
                        </form:select>


                        <form:select path="slot" cssClass="border p-2">
                            <form:option value="morning"   label="Sáng"/>
                            <form:option value="afternoon" label="Chiều"/>
                            <form:option value="evening"   label="Tối"/>
                        </form:select>


                        <form:select path="subjectId" cssClass="border p-2">
                            <c:forEach var="sub" items="${subjectList}">
                                <option value="${sub.subjectId}">${sub.name}</option>
                            </c:forEach>
                        </form:select>
                                
                        <form:hidden path="tutorId" value="${loggedTutorId}"/>
